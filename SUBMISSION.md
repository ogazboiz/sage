...
> Sage uses LI.FI on two real paths. **First, Composer route quotes** for cross-chain onboarding into a Solana app: the Bridge tab quotes a route from Base, Arbitrum, Optimism, Polygon, or Ethereum USDC into the user's Sage vault PDA on Solana via `https://li.quest/v1/quote`, returning route name, fees, slippage, and ETA. The voice agent's `propose_bridge` tool ("Bridge 50 USDC from Base into my vault") opens the Bridge tab pre-filled. **Second, LI.FI Earn data reads** via `https://earn.li.fi/v1/vaults`: every `/brief`, `/yield-snapshot`, and `/alert-check` paid x402 endpoint walks Earn pagination at request time, ranks the cross-chain USDC universe by safety, and returns the result. The HexKit ranking engine is lifted into Sage (`app/src/lib/ranker/`, source files note *"Lifted and trimmed from HexKit"*) — same risk filter (reward-heavy / apy-spike / declining-yield / micro-tvl), same symbol aliases (USDC / USDC.E / USDBC), same ranker. **The agent's payment settles on Solana; the routes and data it surfaces are cross-chain by design.** Every iteration in the Activity log carries an `LI.FI Earn` badge. Solves a concrete user problem: cross-chain onboarding into a Solana-native autonomous-agent app.

**Why it qualifies:**
- Both LI.FI **Composer** (real route quotes via `/v1/quote` for cross-chain bridging into the Sage vault, voice-driven via `propose_bridge`) and **Earn** (load-bearing in three of four paid endpoints, badge visible per iteration).
- Counts under their criteria: real quote, real route, agent-assisted transaction flow, all three.
