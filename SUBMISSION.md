...
> Sage's autonomous agent is a paid consumer of LI.FI Earn. Every `/brief`, `/yield-snapshot`, and `/alert-check` call (three of our four paid x402 endpoints) pulls live data from `https://li.quest/v1/vaults` at request time and returns it as the agent's response. The HexKit ranking engine is lifted into Sage (`app/src/lib/ranker/`, source files note *"Lifted and trimmed from HexKit"*) — same risk filter, same symbol aliases, same ranker. The agent pays USDC on Solana to read LI.FI's cross-chain yield universe, with every iteration in the Activity log carrying an `LI.FI Earn` badge to make the data path visible. The user can also speak *"what's idle?"* and the voice agent calls a free `find_idle_assets` tool that uses the same LI.FI ranker against the connected wallet, suggesting where unused USDC could earn. LI.FI is the data layer the agent pays to access, and the more autonomous tasks run, the more LI.FI calls get triggered.
